...
to-report verifyProposition [ _currentVal _newVal _propositionIndex _breed _currentWho _sourceWho]
  ...
  ;; if the turtle has no prior info on P, use turtle ranking to decide what to do
  if (_currentVal = 0)
  [
  ...
  [
    ...
    ifelse (sourceApperception >= currentApperception)
    [
      let consensus neighborConsensus _propositionIndex _newVal
      ;; check neighbors -  if they agree, change mind
      ifelse (consensus = true) [ report _newVal ]
...
to-report neighborConsensus [ _propositionIndex _truthVal ]
  let my_apperception item _propositionIndex apperception
  let total_neighbors_with_opinion count link-neighbors with [item _propositionIndex propositions != 0 and item _propositionIndex apperception <=  my_apperception]
  let total_neighbors_with_same_opinion count link-neighbors with [item _propositionIndex propositions = _truthVal and item _propositionIndex apperception <=  my_apperception]
...
contradictory-discovery</setup>
  ...
  <experiment name="4thconfig_10nodesNEWMODEL_TOTAL_coverage08" repetitions="10" runMetricsEveryStep="true">
    <go>run-sim 0 1</go>
    <timeLimit steps="1000"/>
