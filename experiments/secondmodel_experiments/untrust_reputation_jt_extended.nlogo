...
to knowProposition [ _propositionIndex _source _truthValue ]
  ...
  let currentApperception calculateApperceptionValue (item _propositionIndex propositionRanking) (proposition_timestamp + 1)
  ...
  [
    ...
    let sourceApperceptionList item 0 [apperception] of turtles with [ who = _source]
    ;; from the apperception list of the source turtle, read the info about the current P
    let sourceApperception item _propositionIndex sourceApperceptionList
    set finalTruthValue verifyProposition currentTruthValue _truthValue currentApperception sourceApperception breed
    if (_is-debug = true) [
      print (word "Apperception of source turtle: " sourceApperception " , apperception of current turtle: " currentApperception)
      print (word "Old truth val: " currentTruthValue " , proposed truth val: " _truthValue " final: " finalTruthValue)
    ]
  ]
  ...
  if (_is-debug = true) [
    ...
    print word "ranking " propositionRanking
    print word "apperception " apperception
    print word "breed " breed
  ]
  ;; change color depending on truth value for viz purposes
  ifelse (finalTruthValue = 1) [ set color green ]
  [ set color magenta ]
end

to-report verifyProposition [ _currentVal _newVal _currentApperception _sourceApperception _breed]
  ;; if it is an attacker, it won't change is mind
  if (_breed = attackers) [ report _currentVal ]
  ;; if the turtle has no prior info on P, just return the new Val
  ...
  [
    ...
    ifelse (_currentApperception >= _sourceApperception) [ report _currentVal ]
    ;; report new value vice versa
    [ report _newVal ]
  ]
end
...
to-report calculateApperceptionValue [ _propositionRanking _tick ]
  report (1.0 - _propositionRanking) * (_tick * 1.0)
end
...
@#$#@#$#@
<experiments>
  <experiment name="experimentVanet1stconfigex3" repetitions="10" runMetricsEveryStep="false">
    <go>run-sim 0 1</go>
